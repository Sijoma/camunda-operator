...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		By("installing eck-operator")
		Expect(utils.InstallEckOperator()).To(Succeed())
	...
	AfterAll(func() {
		...
		By("uninstalling the eck-operator bundle")
		utils.UninstallEckOperator()
	...
	Context("Manager", func() {
		...
		It("can create Camunda", func() {
			By("deploying elasticsearch cluster & camunda")
			cmd := exec.Command("kubectl", "apply", "-f", "test/e2e/manifests")
			_, err := utils.Run(cmd)
			...
			verifyCamundaUp := func(g Gomega) {
				...
				// Validate pod status
				cmd = exec.Command("kubectl", "get",
					"pods", camundaPodName, "-o", "jsonpath={.status.phase}",
					"-n", "default",
				)
				status, err := utils.Run(cmd)
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(status).Should(ContainSubstring("Running"))
			}
